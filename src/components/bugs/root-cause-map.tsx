...
export function RootCauseMap() {
  const nodes = [
    { label: \"SYMPTOM\", text: \"Login request hangs continuously\", icon: AlertCircle, color: \"text-ember\" },
    { label: \"TRIGGER\", text: \"Expired session token passed to auth middleware\", icon: Zap, color: \"text-signal-warning\" },
    { label: \"EVIDENCE\", text: \"401 Unauthorized from /api/session/refresh\", icon: FileSearch, color: \"text-signal-blue\" },
    { label: \"ROOT CAUSE\", text: \"Refresh token fails to renew prior to expiration check\", icon: Target, color: \"text-signal-success\" },
  ];
